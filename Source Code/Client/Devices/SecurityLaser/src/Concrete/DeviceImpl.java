...
public class DeviceImpl implements IDevice {
    ...
    private GpioPinDigitalOutput pinLaser;

    public DeviceImpl(String type, String laserGpio) {
        this.type = type.toLowerCase();
        this.acceptedParams = new AcceptedParams();
        try {
            this.acceptedParams.addData("laser=0:boolean;");
        } catch (Exception e) {
            e.printStackTrace();
        }
        this.paramsVal = new Params();
        this.paramsVal.put("laser", "false");
        this.isOpened = false;

        final GpioController gpio = GpioFactory.getInstance();
        pinLaser = gpio.provisionDigitalOutputPin(RaspiPin.getPinByName(laserGpio), "", PinState.LOW);
    ...
    private void executeCommand() {
        for (Map.Entry<String, String> entry: paramsVal.entrySet()) {
            ...
            this.pinLaser.setState(currentState);
        }
    }
