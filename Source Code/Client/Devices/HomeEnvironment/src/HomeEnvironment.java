...
public class HomeEnvironment {
    public static void main(String[] args) {
        if (args.length != 4) {
            System.out.println("Usage <Device type> <ip> <port> <environment gpio>.");
            return;
        }
        ...
        if (!pattern.matcher(args[3]).matches()) {
            System.out.println("environment gpio invalid.");
            return;
        }
        System.out.println("environment gpio " + args[3]);
        ...
        CommunicationManager com = new CommunicationManager(ip, port, new DeviceImpl(deviceType, Integer.parseInt(args[3])));
        com.doWork();
    }
}
